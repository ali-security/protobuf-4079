...
class PROTOBUF_EXPORT PROTOBUF_ALIGNAS(8)
 ...
 public:
  ...
  // contain of those those types, it can be constructed from them, and you can
...
class
 ...
 public:
  ...
  UniquePtr& operator=(UniquePtr<U>&& rhs) {
    ptr_ = UniquePtrType(rhs.release(), Deleter{rhs.GetOwningArena()});
    return *this;
  }
  ...
  absl::optional<PROTOBUF_NONNULL std::unique_ptr<T>> try_heap_release() {
    ...
    std::unique_ptr<T> ret(ptr_.release());
    ptr_ = UniquePtrType();
    return ret;
  }
